...
| **프로토타입** | 인스턴스 객체가 메서드 참조 할 수ㅇ ..
 - 프로토타입 체인의 대상이 됨. | 인스턴스 객체가 메서드 참조 할 수X ..
(클래스 자체에서 직접 호출해야 함) ..
 - 프로토타입 체인의 대상이 되지 않음. |
| **메서드 내에서 ..
 `this`의 의미** | 인스턴스 객체 (메서드 호출한 객체) | 클래스 함수 (메서드 호출한 객체) |
...
ES5에서는 클래스 상속에 대한 명시적인 문법이 없었다. ..
그래서 프로토타입 기반 상속을 사용하여 클래스처럼 동작하는 구조를 만들 뿐 이였다.
...
2.  `super`: 부모 클래스의 메서드나 생성자를 참조...

    a. super 호출시 - `constructor` 내부에서의 `super()`..

    - 부모 클래스의 constructor가 호출됨. (Rectangle 클래스의 데이터 초기화 로직 그대로 실행됨) - `constructor` 내부에서 `this`를 사용하려면 `super()`를 반드시 먼저 호출해야 한다.
...
수퍼클래스와 서브클래스는 “인스턴스 객체”의 프로토타입 체인 뿐 아니라, ..

“클래스” 간의 프로토타입 체인도 생성함...
